...
test('by default all tests are picked', async () => {
...
test("does not require user to defined Vitest's tags", async () => {
...
test('works even if user defines their own custom tags', async () => {
  await runFixture(
...
test("no tests are picked when tag doesn't match any", async () => {
  await runFixture({ include, tags }, { tags: ['example-5'] });

  expect(getSnapshottedTests()).toMatchInlineSnapshot(`[]`);
});

test('includes tests matching a single tag', async () => {
...
test('includes tests matching multiple tags', async () => {
...
test('includes tests matching tag of describe()', async () => {
...
test('includes tests matching tag of nested describe()', async () => {
...
test('cleans up internal test meta properties', async () => {
  ...
  const metas = tests.map((test) => test.meta());
